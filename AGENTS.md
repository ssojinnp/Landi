...
- 앱 브랜드 로고/파비콘은 `public/landi-mark.svg`, `public/favicon.svg`를 기준으로 사용한다. 앱 로고 자리에 범용 아이콘(`Trees` 등)을 대신 쓰지 않는다.
- 파비콘과 로고 마크는 Primary green `#4f8738` 기반으로 유지하고, 브라우저 탭/앱 헤더/접힘 레일에서 같은 브랜드 신호를 공유한다.
