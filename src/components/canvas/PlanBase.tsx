...
const mutedPalette = defaultPalette.map((plant) => ({
  ...plant,
  colors: { primary: '#8d9588', secondary: '#b4bbad', accent: '#d6dad1', stroke: '#6d7568' },
}))
...
export function PlanBase({ fade = 62 }: PlanBaseProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#f7f7f2]" style={{ opacity: baseOpacity }}>
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 1000 650" preserveAspectRatio="none" aria-hidden="true">
        <path d="M118 70 L875 50 L918 585 L128 585 Z" fill="#d9ddd4" stroke="#b1b8ad" strokeWidth="3" />
        ...
        <path d="M472 165 L571 165 L571 262 L472 262 Z" fill="#b8b2a7" opacity="0.58" stroke="#827b70" strokeWidth="2" />
        ...
        <circle cx="712" cy="178" r="46" fill="#a8aea2" opacity="0.38" stroke="#737b70" strokeWidth="2" />
        <circle cx="826" cy="292" r="48" fill="#adb3a8" opacity="0.34" stroke="#737b70" strokeWidth="2" />
      </svg>
      {hedgePoints.map(([x, y, size], index) => <div key={`hedge-${index}`} className="absolute opacity-20" style={{ left: `${x}px`, top: `${y}px`, transform: 'translate(-50%, -50%)' }}><PlantSymbol plant={{ ...mutedPalette[index % 3], size }} /></div>)}
      {borderShrubs.map(([x, y], index) => <div key={`shrub-${index}`} className="absolute opacity-20" style={{ left: `${x}px`, top: `${y}px`, transform: 'translate(-50%, -50%)' }}><PlantSymbol plant={{ ...mutedPalette[2], size: 34 }} /></div>)}
    </div>
  )
}
